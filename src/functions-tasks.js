...
function memoize(func) {
  const memo = new Map();
  const keyFuncToMemo = func.toString();
  return function innerFn(...rest) {
    if (!memo.has(keyFuncToMemo)) {
      const result = func(...rest);
      memo.set(keyFuncToMemo, result);
    }
    return memo.get(keyFuncToMemo);
  };
}
...
function retry(func, attempts) {
  return function innerFn() {
    ...
    throw new Error(`Error occurred: ${resultError}`);
  };
}
...
function logger(func, logFunc) {
  ...
  return function innerLogger(...rest) {
    const argString = rest.reduce(
      (acc, arg) => acc + (acc ? ',' : '') + JSON.stringify(arg),
      ''
    );
    const startMsg = `${functionName}(${argString}) starts`;
    const endMsg = `${functionName}(${argString}) ends`;
    logFunc(startMsg);
    const result = func(...rest);
    logFunc(endMsg);
    return result;
...
function getIdGeneratorFunction(startFrom) {
  ...
  return function innerFn() {
    const currentCounter = startFromInner;
    startFromInner += 1;
